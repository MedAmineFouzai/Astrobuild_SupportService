...
struct UserStream<T: Sync + Send + Clone + 'static>(usize, UnboundedReceiver<T>);
...
impl<T: Sync + Send + Clone + 'static> Drop for UserStream<T> {
    fn drop(&mut self) {
        with_senders::<T, _, _>(|senders| senders.0.remove(self.0));
    }
}

impl<T: Sync + Send + Clone + 'static> Stream for UserStream<T> {
    type Item = T;
